var paperLayout = {
	...
	createPaper : function (medium) {
		...
		this.parseBibtex();
		this.createColumn();
		this.replaceCiteEntries();
		this.setupColumns();
		this.createHeadlineCounter();
		this.createListCounter();
		this.alignFootnotes();

		fullPageImg = this.getFullPageImage();
	...
	initialize : function() {
		...
		this.getStylesheet();
		this.setupPage();
		this.calcFrontColumnHeight();
		this.calcTextlineHeight();
	},
	...
	calcFrontColumnHeight : function() {
	...
	getColumnHeight : function() {
		...
		switch (this.currentContainer){
			case 0: 
				columnHeight = this.frontColumnHeightPX - this.copyrightBoxHeight;
			break;

			case 1: 
				columnHeight = this.frontColumnHeightPX;
			break;
	...
	getFirstMultiColumnElement : function() {
		...
		switch(this.type){
			...
			case "infovis": 
				if(document.getElementsByClassName('diamondRule')[0]){
	...
	fillColumns : function(element, mode) {
		...
		while (element) {
			...
			remainingSpace = this.calcRemainingSpace(mode);
			if (mode === "normal"){
	...
	getFullPageImage : function() {
		var img;
		if(this.type === "chiextended" && this.medium === "print"){
			img = document.getElementById("fullPageImg");
		}
		return img;
	},

	replaceCiteEntries : function() {
		var element = document.body.firstElementChild;
		while(element) {		
	...
	replaceCite : function(node) {
		var nodeChild, nextNodeChild, textnode, link, j;
		var figCounter = 0;
		var tabCounter = 0;
		var mathCounter = 0;
		var footCounter = 0;
